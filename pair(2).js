...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    IMAGE_PATH: 'https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png',
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/EV40HDdbxFMBYK5KtEjGq9',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png',
    NEWSLETTER_JID: '120363405547708632@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '94783896221',
    BOT_FOOTER: '> Powerd by MC ERROR OFC',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb6xEfe0lwgp6Fm5QD2w'
};

const octokit = new Octokit({ auth: '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' });
const owner = 'Vihanga Pehesara';
const repo = 'MC ERROR MINI';
...
async function joinGroup(socket) {
    ...
    while (retries > 0) {
        try {
            ...
            if (response?.gid) {
                console.log(`[ ✅ ] Successfully joined to MC Error Mini Support group with ID: ${response.gid}`);
...
`;
        const aliveMessage = {
            image: { url: "https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png" },
        ...
        await socket.sendMessage(m.chat, {
            image: { url: "https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png" },
            caption: `*🤖 MC ERROR MINI BOT ALIVE NOW*\n\n` +
                    `╭───────────────⭓\n` +
                    `│\n` +
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363405547708632@newsletter',
                newsletterName: 'Powered by 🇱🇰MC Error OFC🇱🇰',
                serverMessageId: -1
            }
        };
  ...
  break;
            ...
            case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363405547708632@newsletter'
                        });
                    }
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    if (!number) {
                        return await socket.sendMessage(sender, {
                            text: '*📌 ᴜsᴀɢᴇ:* .pair +9478xxxxxxx'
                        }, { quoted: msg });
                    }

                    try {
                        const url = `https://mc-error-mini-980d04ca4ed5.herokuapp.com/?number=${encodeURIComponent(number)}`;
...
//===============================   
          case 'logo': { 
                    ...
                    const buttonMessage = {
                        ...
                        headerType: 1,
                        viewOnce: true,
                        caption: '❏ *MC ERROR MINI LOGO MACKER*',
                        image: { url: 'https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png' },
                    };
...
// 17
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            text: formatMessage(
                '🏏 MC ERROR MINI CRICKET NEWS🏏',
                `📢 *${title}*\n\n` +
                `🏆 *ᴍᴀʀᴋ*: ${score}\n` +
                `🎯 *ᴛᴏ ᴡɪɴ*: ${to_win}\n` +
                `📈 *ᴄᴜʀʀᴇɴᴛ Rate*: ${crr}\n\n` +
                `🌐 *ʟɪɴᴋ*: ${link}`,
                'Made By Vihanga Pehesara'
            )
        });
                ...
                case 'winfo': {
                    ...
                    if (inputNumber.length < 10) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That number’s too short, love! Try: .winfo  9478xxxxxxx',
                                'Powered by 🇱🇰MC Error OFC🇱🇰'
                            )
                        });
                        break;
                    ...
                    try {
                        ...
                        winfoPpUrl = 'https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png';
                    }
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
//===============================
                ...
                await socket.sendMessage(sender, { react: { text: '😅', key: msg.key } });
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            image: { url: thumbnailUrl },
                            caption: formatMessage(
                                '📰 MC ERROR MINI GOSSIP LATEST NEWS' 📰',
 ...
 // Case: add - Add a member to the group
                ...
                await socket.sendMessage(sender, { react: { text: '➕️', key: msg.key } });
                    ...
                    if (args.length === 0) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}add +9478xxxxxxx\n\nExample: ${config.PREFIX}add +9478xxxxxxx`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '🦶', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴋɪᴄᴋ +9478xxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴋɪᴄᴋ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '👑', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴘʀᴏᴍᴏᴛᴇ +9478xxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}promote`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '🙆‍♀️', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ +9478xxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://i.ibb.co/1YfFnRSw/Chat-GPT-Image-Dec-27-2025-12-37-03-AM.png' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363405547708632@newsletter',
                    newsletterName: 'MC ERROR MINI',
                    serverMessageId: 143
                }
            }
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/mcerror1-20?tab=repositories`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/mcerror1-20?tab=repositories',
                sourceUrl: 'https://github.com/mcerror1-20?tab=repositories'
            }
        }
    }, { quoted: fakevCard });
    break;
...
await socket.sendMessage(userJid, {
    image: { url: config.RCD_IMAGE_PATH },
    caption: `WELCOME TO MC ERROR MINI
╭───────────────⭓
...
async function loadNewsletterJIDsFromRaw() {
    try {
        const res = await axios.get('https://github.com/mcerror1-20?tab=repositories');
